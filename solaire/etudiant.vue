...
<th>Date Naissance</th>
        ...
        <tbody>
          <tr v-for="(etudiant, id) in etudiants" :key="id">
            <th>{{ etudiant.etu }}</th>
<th>{{ etudiant.nom }}</th>
...
<th>{{ etudiant.dateNaissance }}</th>
    ...
    <dialog id="dialog-etudiants"> <!-- Ovaina par rapport am anarany ilay tableschema -->
      <p>Add new etudiant</p>
    ...
    <input id="prenom" type="text" name="prenom" v-model="prenom">
</div><div class="input-group">
    <label for="dateNaissance">dateNaissance:</label>
    <input id="dateNaissance" type="text" name="dateNaissance" v-model="dateNaissance">
</div>
...
dateNaissance: '',
  ...
  methods: {
    ...
    async fetchItems() {
      const response = await axios.get('http://localhost:8080/classe-1.0-SNAPSHOT/etudiants.do')
...
dateNaissance: this.dateNaissance,
      ...
      const response = await fetch(
        'http://localhost:8080/classe-1.0-SNAPSHOT/etudiants.do',
        request
      )
...
this.dateNaissance = ''
    ...
    editFramework(index) {
      ...
      const item = this.etudiants[index];
      this.etu = item.etu
this.nom = item.nom
this.prenom = item.prenom
this.dateNaissance = item.dateNaissance
  ...
  mounted() {
    this.dialog = document.getElementById("dialog-etudiants")
    this.fetchItems()
  }
}
