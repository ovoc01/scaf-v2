...
<th>Nom</th>
        ...
        <tbody>
          <tr v-for="(option, id) in options" :key="id">
            <th>{{ option.idOption }}</th>
<th>{{ option.nom }}</th>
    ...
    <dialog id="dialog-options"> <!-- Ovaina par rapport am anarany ilay tableschema -->
      <p>Add new option</p>
...
nom: '',
  ...
  methods: {
    ...
    async fetchItems() {
      const response = await axios.get('http://localhost:8080/classe-1.0-SNAPSHOT/options.do')
...
nom: this.nom,
      ...
      const response = await fetch(
        'http://localhost:8080/classe-1.0-SNAPSHOT/options.do',
        request
      )
...
this.nom = item.nom
  ...
  mounted() {
    this.dialog = document.getElementById("dialog-options")
    this.fetchItems()
  }
}
