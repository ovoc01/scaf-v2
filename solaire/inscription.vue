...
<th>Id Semestre</th>
        ...
        <tbody>
          <tr v-for="(inscription, id) in inscriptions" :key="id">
            <th>{{ inscription.idInscription }}</th>
<th>{{ inscription.date }}</th>
<th>{{ inscription.etu }}</th>
<th>{{ inscription.idSemestre }}</th>
    ...
    <dialog id="dialog-inscriptions"> <!-- Ovaina par rapport am anarany ilay tableschema -->
      <p>Add new inscription</p>
      <div class="form-container">
        <form>
          <div class="input-group">
    <label for="date">date:</label>
    <input id="date" type="text" name="date" v-model="date">
</div>
...
idSemestre: '',
  ...
  methods: {
    ...
    async fetchItems() {
      const response = await axios.get('http://localhost:8080/classe-1.0-SNAPSHOT/inscriptions.do')
...
idSemestre: this.idSemestre,
      ...
      const response = await fetch(
        'http://localhost:8080/classe-1.0-SNAPSHOT/inscriptions.do',
        request
      )
...
this.idSemestre = ''
    ...
    editFramework(index) {
      ...
      const item = this.inscriptions[index];
      this.idInscription = item.idInscription
this.date = item.date
this.etu = item.etu
this.idSemestre = item.idSemestre
  ...
  mounted() {
    this.dialog = document.getElementById("dialog-inscriptions")
    this.fetchItems()
  }
}
