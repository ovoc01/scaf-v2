...
<th>{{ typeArticle.description }}</th>
    ...
    <dialog id="dialog-typeArticles"> <!-- Ovaina par rapport am anarany ilay tableschema -->
      <p>Add new typeArticle</p>
...
description: '',
  ...
  methods: {
    ...
    async fetchItems() {
      const response = await axios.get('http://localhost:8080/typeArticles.do')
...
description: this.description,
      ...
      const response = await fetch(
        'http://localhost:8080/typeArticles.do',
        request
      )
      const data = await response.json()
      this.typeArticles.push(data)
    },
...
this.description = ''
    ...
    editFramework(index) {
      ...
      const item = this.typeArticles[index];
      this.id = item.id
...
this.typeArticles = response.data.typeArticles
  ...
  mounted() {
    this.dialog = document.getElementById("dialog-typeArticles")
    this.fetchItems()
  }
}
